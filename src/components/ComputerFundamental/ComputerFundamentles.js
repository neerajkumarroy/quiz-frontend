...
const ComputerFundamentals = () => {
    ...
    const [timeRemaining, setTimeRemaining] = useState(780); // 660 seconds for 11 minutes
    const [showExplanations, setShowExplanations] = useState({});
    const [showPopup, setShowPopup] = useState(false);
    const [showInitialPopup, setShowInitialPopup] = useState(true); // State to manage initial popup visibility

    const [showSlider, setShowSlider] = useState(false); // Slider state
    ...
    const handleSubmit = () => {
        ...
        navigate('/computer-fundamentals-results', { state: { totalQuestions, correctCount, incorrectQuestions, percentage, attemptedQuestions } });
    };
    ...
    const handleToggleSlider = () => {
        setShowSlider(!showSlider); // Toggle slider visibility
    };
...
export default memo(ComputerFundamentals);
